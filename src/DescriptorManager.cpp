...
void DescriptorManager::createSamplerLayout() {
    ...
    if (vkCreateDescriptorSetLayout(m_device, &layoutInfo, nullptr, &m_samplerLayout) != VK_SUCCESS) {
...
void DescriptorManager::updateUBODescriptor(VkDescriptorSet descriptorSet, VkBuffer buffer, VkDeviceSize bufferSize) {
    if (m_device == VK_NULL_HANDLE) {
...
void DescriptorManager::updateTextureDescriptor(VkDescriptorSet descriptorSet, VkImageView imageView, VkSampler sampler) {
    if (m_device == VK_NULL_HANDLE) {
        throw std::runtime_error("DescriptorManager device not set!");
    }
