...
ShaderCache::ShaderCache(const std::string& cacheDirectory)
    ...
    , m_manifestPath(cacheDirectory + "/manifest.json")
    ...
    try {
        ...
        while ((pos = content.find("\"sourcePath\"", pos)) != std::string::npos) {
            ...
            if (hashPos != std::string::npos) {
                ...
                entry.sourceHash = ShaderHash::fromHexString(
                    entryStr.substr(hashStart, hashEnd - hashStart)
                );
            }
            ...
            if (stagePos != std::string::npos) {
                ...
                if (stageName == "vertex") entry.stage = ShaderStage::Vertex;
                else if (stageName == "fragment") entry.stage = ShaderStage::Fragment;
                else if (stageName == "compute") entry.stage = ShaderStage::Compute;
                else if (stageName == "geometry") entry.stage = ShaderStage::Geometry;
                else if (stageName == "tess_control") entry.stage = ShaderStage::TessControl;
                else if (stageName == "tess_eval") entry.stage = ShaderStage::TessEvaluation;
            }
...
bool ShaderCache::saveManifest() {
    ...
    for (const auto& [path, entry] : m_entries) {
        ...
        switch (entry.stage) {
            case ShaderStage::Vertex:         stageName = "vertex"; break;
            case ShaderStage::Fragment:       stageName = "fragment"; break;
            case ShaderStage::Compute:        stageName = "compute"; break;
            case ShaderStage::Geometry:       stageName = "geometry"; break;
            case ShaderStage::TessControl:    stageName = "tess_control"; break;
            case ShaderStage::TessEvaluation: stageName = "tess_eval"; break;
...
std::vector<uint32_t> ShaderCache::loadShader(
...
bool ShaderCache::saveSpvToDisk(
...
std::vector<uint32_t> ShaderCache::compileAndCache(
    const std::string& sourcePath,
    ShaderStage stage)
{
    // Compile shader
