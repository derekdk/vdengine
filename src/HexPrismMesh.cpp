...
void HexPrismMeshGenerator::generate(
    float hexRadius,
    std::vector<HexPrismVertex>& outVertices,
...
void HexPrismMeshGenerator::generateTopFace(
    float radius,
    std::vector<HexPrismVertex>& verts,
...
void HexPrismMeshGenerator::generateBottomFace(
    float radius,
    std::vector<HexPrismVertex>& verts,
    std::vector<uint32_t>& indices,
    uint32_t baseIndex
...
void HexPrismMeshGenerator::generateSideFaces(
    float radius,
    std::vector<HexPrismVertex>& verts,
    std::vector<uint32_t>& indices,
    uint32_t baseIndex
    ...
    for (int i = 0; i < 6; ++i) {
        ...
        glm::vec3 normal = glm::normalize(glm::cross(edge2, edge1));  // Swapped order for outward normal
        ...
        if (uRight == 0.0f) uRight = 1.0f;  // Wrap around for last face
