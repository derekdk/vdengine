...
void Game::createSpriteRenderingPipeline() {
    ...
    if (!vertResult.success) {
        std::cerr << "Sprite vertex shader compilation failed: " << vertResult.errorLog << std::endl;
    ...
    if (!fragResult.success) {
        throw std::runtime_error("Failed to compile sprite fragment shader: " + fragResult.errorLog);
    }
    
    // Create shader modules
    VkShaderModule vertShaderModule = m_vulkanContext->createShaderModule(
        std::vector<char>(reinterpret_cast<char*>(vertResult.spirv.data()),
                         reinterpret_cast<char*>(vertResult.spirv.data()) + vertResult.spirv.size() * sizeof(uint32_t))
    );
    VkShaderModule fragShaderModule = m_vulkanContext->createShaderModule(
        std::vector<char>(reinterpret_cast<char*>(fragResult.spirv.data()),
                         reinterpret_cast<char*>(fragResult.spirv.data()) + fragResult.spirv.size() * sizeof(uint32_t))
    );
    ...
    if (vkCreateDescriptorSetLayout(device, &layoutInfo, nullptr, &m_spriteDescriptorSetLayout) != VK_SUCCESS) {
    ...
    pushConstantRange.stageFlags = VK_SHADER_STAGE_VERTEX_BIT;
    pushConstantRange.offset = 0;
    pushConstantRange.size = sizeof(glm::mat4) + sizeof(glm::vec4) + sizeof(glm::vec4);  // model + tint + uvRect
    ...
    if (vkCreatePipelineLayout(device, &pipelineLayoutInfo, nullptr, &m_spritePipelineLayout) != VK_SUCCESS) {
    ...
    if (vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &m_spritePipeline) != VK_SUCCESS) {
    ...
    if (!m_defaultWhiteTexture->createFromData(whitePixel, 1, 1, device,
...
VkDescriptorSet Game::allocateSpriteDescriptorSet() {
    ...
    if (vkAllocateDescriptorSets(m_vulkanContext->getDevice(), &allocInfo, &descriptorSet) != VK_SUCCESS) {
        return VK_NULL_HANDLE;
    }
...
void Game::updateSpriteDescriptor(VkDescriptorSet descriptorSet, 
