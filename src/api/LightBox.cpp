...
ThreePointLightBox::ThreePointLightBox(const Color& keyColor, float keyIntensity) {
    ...
    Light key =
        Light::directional(Direction(-0.5f, -1.0f, -0.5f).normalized(), keyColor, keyIntensity);
    addLight(key);

    // Fill light - softer light from front-left
    // Fills in shadows created by the key light
    Light fill = Light::directional(Direction(0.5f, -0.5f, -0.5f).normalized(), keyColor,
    ...
    Light back = Light::directional(Direction(0.0f, -0.3f, 1.0f).normalized(), keyColor,
