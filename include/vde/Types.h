...
struct GPULight {
    alignas(16) glm::vec4
        positionAndType;  ///< xyz = position/direction, w = type (0=dir, 1=point, 2=spot)
    alignas(16) glm::vec4
        directionAndRange;  ///< xyz = direction (for spot/dir), w = range (for point/spot)
...
static_assert(sizeof(LightingUBO) == 544, "LightingUBO size must be 544 bytes");
...
static_assert(sizeof(MaterialPushConstants) == 48, "MaterialPushConstants size must be 48 bytes");
