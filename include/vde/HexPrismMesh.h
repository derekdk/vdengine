...
struct HexPrismVertex {
    ...
    HexPrismVertex(const glm::vec3& pos, const glm::vec3& norm, 
                   const glm::vec2& uv, uint8_t face)
        : position(pos), normal(norm), texCoord(uv), faceType(face)
        , padding{0, 0, 0} {}
...
private:
     ...
     * @param baseIndex Starting index for vertices
     */
    static void generateBottomFace(
        float radius,
        std::vector<HexPrismVertex>& verts,
        std::vector<uint32_t>& indices,
        uint32_t baseIndex
